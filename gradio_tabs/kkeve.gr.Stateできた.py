...
def change_language(language_dropdown, list_model_dropdown, list_character_dropdown, character_dropdown, category_dropdown):
	...
	global conf
	if conf['language'] == language_dropdown: 
		components = [
			#gr.update(), 
			gr.update(), 
			gr.update(), 
		...
		conf['language'] = language_dropdown
...
def on_language_change(language_state):
...
def create_interface(language_state) -> gr.Blocks:
	...
	'''
	# 言語
	conf['language'] = get_cache("default", "language")
	if conf['language'] is None:
		conf['language'] = locale.getdefaultlocale()[0][:2]
		update_cache("default", "language", conf['language'])
	'''

	print(language_state.value)
